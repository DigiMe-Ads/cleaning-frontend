...
const isOverdue = (booking) => {
  ...
  const checkoutDateTime = new Date(`${booking.checkout_date}T${booking.checkout_time || '23:59'}`);
  return isPast(checkoutDateTime);
};
...
const AdminDashboard = () => {
  ...
  // Sort: overdue first, then by checkout date/time ascending
      ...
      {overdueCount > 0 && (
        <div className="animate-fadeUp" style={{
          ...
          <p style={{ fontSize: '14px', color: 'var(--danger)', fontWeight: '500' }}>
            {overdueCount} booking{overdueCount !== 1 ? 's are' : ' is'} overdue and not marked as done — scroll down to review.
          </p>
        </div>
      )}
            ...
            {todayBookings.sort((a, b) => a.checkout_time > b.checkout_time ? 1 : -1).map((booking) => {
              ...
              return (
                <div key={booking.id} style={{
                  ...
                  padding: '14px 18px',
                  background: overdue ? 'rgba(176,58,46,0.2)' : 'rgba(255,255,255,0.06)',
                  borderRadius: '14px',
                  border: `1px solid ${overdue ? 'rgba(176,58,46,0.35)' : 'rgba(255,255,255,0.08)'}`,
                  gap: '12px', flexWrap: 'wrap', transition: 'background 0.15s',
                }}
                  onMouseEnter={e => e.currentTarget.style.background = overdue ? 'rgba(176,58,46,0.28)' : 'rgba(255,255,255,0.1)'}
                  onMouseLeave={e => e.currentTarget.style.background = overdue ? 'rgba(176,58,46,0.2)' : 'rgba(255,255,255,0.06)'}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: '12px', color: overdue ? 'rgba(240,149,149,0.7)' : 'rgba(255,255,255,0.38)', marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {overdue ? '⚠ Overdue — cleaning not marked done' : booking.properties?.location}
                      </p>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0, flexWrap: 'wrap' }}>
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ fontSize: '14px', fontWeight: '600', color: overdue ? '#F09595' : 'var(--amber)' }}>
                        {formatTime(booking.checkout_time)}
                      </p>
                      <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)', marginTop: '2px' }}>
                        {booking.cleaners?.name || 'Unassigned'}
                      </p>
                    </div>
                    <CleaningStatusBadge status={booking.cleaning_status} />
                  </div>
                </div>
              );
            })}
      ...
      <div style={{ background: 'var(--ivory)', borderRadius: '20px', border: '1px solid var(--warm-100)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--warm-100)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', color: 'var(--warm-400)', marginTop: '2px' }}>
              Sorted by next upcoming — overdue shown first
            </p>
          </div>
          <button
        ...
        <div className="mobile-booking-cards">
          {sortedBookings.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--warm-300)', fontSize: '14px' }}>
              No bookings yet
            </div>
          ) : sortedBookings.map((booking) => {
            const overdue = isOverdue(booking);
            return (
              <div key={`mobile-${booking.id}`} style={{
                padding: '16px 20px',
                borderTop: '1px solid var(--warm-100)',
                background: overdue ? 'rgba(176,58,46,0.04)' : 'transparent',
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '10px' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      <p style={{ fontSize: '14px', fontWeight: '600', color: overdue ? 'var(--danger)' : 'var(--espresso)' }}>
                        {booking.properties?.name || '—'}
                      </p>
                    </div>
                  ...
                  <button
                    ...
                    style={{ width: '28px', height: '28px', borderRadius: '7px', border: '1px solid transparent', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--warm-200)', transition: 'all 0.15s', flexShrink: 0 }}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '8px' }}>
                  <div>
                    <p style={{ fontSize: '10px', fontWeight: '600', color: 'var(--warm-300)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Booking</p>
                    <select
                      value={booking.status}
                      onChange={(e) => handleStatusUpdate(booking.id, e.target.value)}
                      style={{
                        width: '100%', fontSize: '12px', fontFamily: 'var(--font-body)',
                        borderRadius: '99px', padding: '6px 10px',
                        cursor: 'pointer', outline: 'none',
                        fontWeight: '600', appearance: 'none', textAlign: 'center',
                  ...
                  <div>
                    <p style={{ fontSize: '10px', fontWeight: '600', color: 'var(--warm-300)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Cleaning</p>
                    <select
                      value={booking.cleaning_status || 'unassigned'}
                      onChange={(e) => handleCleaningStatusUpdate(booking.id, e.target.value)}
                      style={{
                        width: '100%', fontSize: '12px', fontFamily: 'var(--font-body)',
                        borderRadius: '99px', padding: '6px 10px',
                        cursor: 'pointer', outline: 'none',
                        fontWeight: '600', appearance: 'none', textAlign: 'center',
                        ...getCleaningStatusStyle(booking.cleaning_status || 'unassigned'),
                      }}
                    >
                  ...
                  <div style={{ gridColumn: '1 / -1' }}>
                    ...
                    <select
                      ...
                      style={{
                        ...
                        padding: '6px 8px', background: 'var(--cream)',
                        color: booking.cleaner_id ? 'var(--espresso)' : 'var(--warm-300)',
                        cursor: 'pointer', outline: 'none',
                      }}
                    >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--warm-100)' }}>
                {['', 'Property', 'Client', 'Checkout', 'Assign Cleaner', 'Booking Status', 'Cleaning Status', ''].map((h, i) => (
            ...
            <tbody>
              {sortedBookings.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ textAlign: 'center', padding: '60px', color: 'var(--warm-300)', fontSize: '14px' }}>
                    No bookings yet
                  </td>
                </tr>
              ) : sortedBookings.map((booking) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '16px 8px 16px 16px', width: '24px' }}>
                      {overdue && (
                        <div title="Overdue — cleaning not marked done">
                    ...
                    <td style={{ padding: '16px 16px' }}>
                      <p style={{ fontSize: '14px', fontWeight: '500', color: overdue ? 'var(--danger)' : 'var(--espresso)' }}>
                        {booking.properties?.name || '—'}
                      </p>
                    ...
                    <td style={{ padding: '16px 16px', whiteSpace: 'nowrap' }}>
                      <p style={{ fontSize: '13px', fontWeight: '500', color: overdue ? 'var(--danger)' : 'var(--espresso)' }}>
